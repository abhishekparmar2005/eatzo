...
const RestaurantCard = ({ restaurant }) => {
  const { _id, name, image, description, location, cuisine, rating,
          deliveryTime, minOrder, isCurrentlyOpen, openTime, closeTime } = restaurant;

  // isCurrentlyOpen comes from backend virtual field
  const open = isCurrentlyOpen ?? true;
    ...
    <Link to={`/restaurant/${_id}`} className="block">
      <div className="card overflow-hidden group cursor-pointer">
        <div className="relative h-48 bg-gray-100 overflow-hidden">
          <img
            src={image || `https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=400&h=300&fit=crop`}
            alt={name}
          ...
          {!open && (
            <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center gap-1">
              ...
              {openTime && <span className="text-white/80 text-xs">Opens at {openTime}</span>}
            </div>
          )}
        ...
        <div className="p-4">
          <div className="flex items-start justify-between mb-1">
            ...
            <span className={`text-xs font-semibold ml-2 flex-shrink-0 ${open ? 'text-green-600' : 'text-red-500'}`}>
              {open ? '● Open' : '● Closed'}
            </span>
          </div>
          ...
          {openTime && closeTime && (
            <p className="text-xs text-gray-400 mt-2">🕐 {openTime} – {closeTime}</p>
          )}
        </div>
      </div>
    </Link>
